...
{
	var count = 0;
	count += unsafeWindow.arrows.length;
	count += unsafeWindow.spears.length;
	count += unsafeWindow.pikes.length;
	return count;
}
...
function bot_Get_Action()
{
	...
	if(unsafeWindow.attack_queue[0].type == 'arrow')
	{
	...
	if(unsafeWindow.attack_queue[0].type == 'spear' && Math.random() <  random_Exploration_Rate)
	{
...
function spear_get_State()
{
	...
	var x = unsafeWindow.heart.sprite.position.x - tip.x;
	var y = unsafeWindow.heart.sprite.position.y - tip.y;
...
function get_closest_arrow()
{
  ...
  for (var a = 0; a < unsafeWindow.arrows.length; ++a)
  {
    var currentPos = unsafeWindow.arrows[a].sprite.position;
    var dist = get_Point_Distance(currentPos.x, currentPos.y, unsafeWindow.heart.sprite.position.x, unsafeWindow.heart.sprite.position.y);
    if (dist < distance['dist'])
    {
      closestID = a;
      distance['dist'] = dist;
      distance['x'] = unsafeWindow.heart.sprite.position.x - currentPos.x;
      distance['y'] = unsafeWindow.heart.sprite.position.y - currentPos.y;
    }
  }
...
function get_closest_spear()
{
	...
	for (var a = 0; a < unsafeWindow.spears.length; ++a)
	{
		var currentSpear = unsafeWindow.spears[a];
	  
		var tip = { x: currentSpear.pos_x + currentSpear.direction.x * 23, y: currentSpear.pos_y + currentSpear.direction.y * 23};
	
		var currentPos = unsafeWindow.spears[a].sprite.position;
		var dist = get_Point_Distance(tip.x, tip.y, unsafeWindow.heart.sprite.position.x, unsafeWindow.heart.sprite.position.y);
		if (dist < distance['dist'])
		{
			closestID = a;
			distance['dist'] = dist;
  ...
  return unsafeWindow.spears[closestID];
}
...
{
	unsafeWindow.handleKeyInput(key, 'down');
	unsafeWindow.handleKeyInput(key, 'up');
}
...
function bot_press_key_and_hold(key)
{
	...
	{
		unsafeWindow.handleKeyInput(bot_pressed_key, 'up');
		bot_pressed_key = key;
		if(key != "none")
			unsafeWindow.handleKeyInput(key, 'down');
	}
...
window.setInterval(function () 
{
	...
	if (unsafeWindow.scene.scene_state == 'splash')
	{
		bot_press_key('A');
	}
  
	if(oldHP != unsafeWindow.heart.hp)
    {
		if(unsafeWindow.heart.hp < oldHP)
        {
			...
			if(unsafeWindow.attack_queue[0].type == 'spear')
            {              
				spearDmg++;
            }            
        }
		oldHP = unsafeWindow.heart.hp;
    }
	previousState = unsafeWindow.gamestate.state;
  
	if (unsafeWindow.scene.scene_state == 'gameplay')
	{    
		if(unsafeWindow.gamestate.state == 'gameover' && !handeledDeath)
		{
			gamesPlayed++;
			if(saveToFile)
				WriteToFile(gamesPlayed, unsafeWindow.gamestate.elapsed_time, qValues, initial_Learn_Rate, bot_Discount);
			handeledDeath = true;                    
		}    
		
		// Skip through menu and gameover prompts
		if (unsafeWindow.gamestate.state == 'menu' || unsafeWindow.gamestate.state == 'gameover')
		{      
			bot_press_key('A');
		} 
		else
...
// File Writing
unsafeWindow.SaveData = function(itter, time, arrowDmg, spearDmg, lr, discount)
{  
	GM_xmlhttpRequest ( 
	{
		method:     "POST",
...
unsafeWindow.SaveQValues = function(QValues, lr, discount)
{  
...
function WriteToFile(itter, time, QValues, lr, discount)
{
	...
	setTimeout(function () 
	{   
		unsafeWindow.SaveQValues(QValues, lr, discount);
	}, 0);
  
	setTimeout(function () 
	{   
		unsafeWindow.SaveData(itter,time, arrowDmg, spearDmg, lr, discount);
	}, 0);  
}
